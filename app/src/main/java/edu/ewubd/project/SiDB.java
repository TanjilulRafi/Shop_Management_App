...
public class SiDB extends SQLiteOpenHelper {

    // TABLE INFORMATTION
    static final String DB_NAME = "KEY_VALU.DB";
    public final String TABLE_KEY_VALUE1 = "key_value_pairs1";
    public final String KEY = "keyname";
    public final String VALUE = "itemvalue";
    //
    public SiDB(Context context) {
    ...
    private void createKeyValueTable(SQLiteDatabase db){
        try {
            db.execSQL("create table " + TABLE_KEY_VALUE1 + " (" + KEY
                    + " TEXT, " + VALUE + " TEXT)");
        } catch (Exception e) {
        }
    }
    ...
    private void handleError(SQLiteDatabase db, Exception e){
        ...
        if (errorMsg.contains("no such table")){
            if (errorMsg.contains(TABLE_KEY_VALUE1)){
                createKeyValueTable(db);
            }
        }
    }
    ...
    public Boolean insertKeyValue(String key, String value) {
        ...
        long result;
        try{
            result = db.insert(TABLE_KEY_VALUE1, null, cv);
        }catch (Exception e){
            handleError(db, e);
            result = db.insert(TABLE_KEY_VALUE1, null, cv);
        }
    ...
    public Cursor getAllKeyValues() {
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("select * from " + TABLE_KEY_VALUE1, null);
        return res;
    }
    ...
    public boolean updateValueByKey(String key, String value) {
        ...
        int nr = 0;
        try{
            nr = db.update(TABLE_KEY_VALUE1, cv, KEY + "=?",
                    new String[] { key });
        }catch (Exception e){
            handleError(db, e);
            try {
    ...
    public Integer deleteDataByKey(String key) {
        ...
        int isDeleted = 0;
        try{
            isDeleted = db.delete(TABLE_KEY_VALUE1, KEY + " = ?", new String[] { key });
        }catch (Exception e){
            handleError(db, e);
            try {
                isDeleted = db.delete(TABLE_KEY_VALUE1, KEY + " = ?", new String[]{key});
    ...
    public String getValueByKey(String key) {
        ...
        Cursor res;
        try{
            res = db.rawQuery("SELECT * FROM " + TABLE_KEY_VALUE1 + " WHERE "
                    + KEY + "='" + key + "'", null);
        }catch (Exception e){
            handleError(db, e);
            res = db.rawQuery("SELECT * FROM " + TABLE_KEY_VALUE1 + " WHERE "
